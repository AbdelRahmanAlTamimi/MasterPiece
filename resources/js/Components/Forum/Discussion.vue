        ...
        <div class="p-6 text-gray-900 flex items-center space-x-6">
            <div class="flex-grow">
                ...
                <Link :href="`${route('discussions.show', discussion)}?post=${discussion.latest_post.id}`" class="inline-block text-sm mt-3">
                    Last post by {{ discussion.latest_post.user?.username || '[previous user]' }} <time :datetime="discussion.latest_post.created_at.datetime" :title="discussion.latest_post.created_at.datetime">{{ discussion.latest_post.created_at.human }}</time>
                </Link>
            </div>
