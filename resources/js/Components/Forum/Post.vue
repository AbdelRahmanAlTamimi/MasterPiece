    ...
    <div :id="`post-${post.id}`"
        ...
        <div class="w-full">
            <div>
                ...
                <div class="text-sm text-gray-500">Posted <time :datetime="post.created_at.datetime"
                        :title="post.created_at.datetime">{{ post.created_at.human }}</time></div>
            </div>
            <div class="mt-3">
            ...
            <ul class="flex items-center space-x-3 mt-6">
                ...
                <li v-if="post.discussion.user_can.solve">
                    <button class="text-indigo-500 text-sm"
        ...
        <div class="absolute right-0 top-0 bg-gray-800 text-gray-100 px-3 py-1 text-xs uppercase tracking-wide font-semibold rounded-bl shadow-sm"
            v-if="isSolution">Best answer</div>
    </div>
</template>
